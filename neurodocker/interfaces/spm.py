...
def _get_mcr_url(matlab_version):
    """Return URL to MATLAB MCR installation file."""
    base = 'https://www.mathworks.com/supportfiles/'

    if matlab_version == "R2010a":
        url = ("http://www.fil.ion.ucl.ac.uk/spm/download/restricted"
               "/utopia/MCR/glnxa64/MCRInstaller.bin")
    elif matlab_version > LooseVersion("R2013a"):
        rel = ('downloads/{ver}/deployment_files/{ver}/installers/'
               'glnxa64/MCR_{ver}_glnxa64_installer.zip')
        url = urljoin(base, rel).format(ver=matlab_version)
    elif matlab_version >= LooseVersion("R2012a"):
        rel = 'MCR_Runtime/{ver}/MCR_{ver}_glnxa64_installer.zip'
        url = urljoin(base, rel).format(ver=matlab_version)
    else:
        raise ValueError("Cannot find URL for requested MCR version.")
...
class SPM(object):
    ...
    def __init__(self, version, matlab_version, pkg_manager, check_urls=True):
        ...
        if self.version != '12':
            raise ValueError("Only SPM12 is supported at this time.")
    ...
    def install_spm(self):
        ...
        spm_cmd = '/opt/spm{0}/run_spm{0}.sh {1}'.format(self.version,
                                                         mcr_path)
